...
func TestParseTestCase(t *testing.T) {
	tc := testCase{}
	err := tc.ParseTestCase("testcases/random", "testcases/random/test.1.in", "testcases/random/test.1.out")
	...
	err = tc.ParseTestCase("testcases/random", "test.1.in", "test.1.out")
	require.Error(t, err)
	fmt.Println(tc)
}
