...
func scheduler(ctx context.Context, caseResCh <-chan caseResult) chan testResult {
	testCh := make(chan testResult)
	testRes := make(resultTable)

	go storeResult(ctx, testRes, caseResCh, testCh)

	return testCh
}

func storeResult(ctx context.Context, res resultTable, caseCh <-chan caseResult, testCh chan<- testResult) {
	for {
		select {
		case <-ctx.Done():
		...
		case r := <-caseCh:
			...
			if res[r.sc.arrayType][r.sc.sortMethod] == nil {
				res[r.sc.arrayType][r.sc.sortMethod] = make([]hw07.SortTime, sizeCount)
			}
